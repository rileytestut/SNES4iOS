...
/*    	GFX.ScreenColors = &IPPU.ScreenColors [(((Tile >> 10) & BG.PaletteMask) << BG.PaletteShift) + BG.StartPalette];*/
    ...
    //Tile is not blank, 'have to draw it  
		Offset-=(320*StartLine);

		/*
			Funky inline assembler - keep for ref
			asm volatile (
				"add %0,%0,#8\n\t"
				"mov r1,#8\n\t"
				"ldmia %0!,{r3,r12}\n\t"
	    ...
	    if (!(Tile & (V_FLIP | H_FLIP)))
	    {
		    //NO FLIP
#ifdef asmPPU8
			asmDrawTile8((uint32)pCache,0|(8<<16),(uint32)(GFX.S+(Offset)),0);
#else
			bp = pCache+8;
			for (l = 8; l != 0; l--, bp += 8, Offset+=320)
