...
unsigned char InitFramebuffer=0;
int Timer=0;
...
{
	// Check if we are already in requested bpp mode
...
{
	/*unsigned short *screen = (unsigned short*)ddlayer;//Framework2D_LockBuffer();
	unsigned short *pix = (unsigned short*)framebuffer16[0];
	int x=0,y=0;
	for(y=0;y<240;y++)
	{
		for(x=0;x<320;x++)
...
void gp_initSound(int rate, int bits, int stereo, int Hz, int frag)
{
	...
	/*while(1)
	{
		memset(framebuffer8[0],0,320*240);
		x=FrameworkAudio_GetCurrentBank();
		sprintf(text,"Current bank: %x",x);
		gp_drawString(0,60,strlen(text),text,0xFFFF,framebuffer16[0]);
		MenuFlip();
	}*/
}
